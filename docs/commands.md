...
* **radio** `[id]`
    ...
    - Stations are configured by the server owner in `config/radio.json`.
    - Requires the `can_radio` area pref to be enabled (off by default) and respects the area's music prefs (`can_dj`, `music_locked`, etc.).
* **blockdj** `<id>` *(Mod)*
